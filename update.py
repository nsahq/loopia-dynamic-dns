...
def get_records(config: dict, record_type: list = ['A']) -> list:
    ...
    try:
        ...
        if 'AUTH_ERROR' in zone_records:
            raise ConnectionError('UNAUTHORIZED: Unable to authenticate. Please check your user/pass.')

        # Quit if API returns unknown error
        if 'UNKNOWN_ERROR' in zone_records:
            raise ConnectionAbortedError('UNKNOWN ERROR: Could not process request or requested (sub)domain does not exist in this account.')
...
def add_record(config: dict, ip: str):
...
def update_record(config: dict, ip: str, record: dict):
    """Update an existing A record for a subdomain in a zone"""

    if record['rdata'] != new_ip and record['ttl'] != config['ttl']:
        new_record = {
